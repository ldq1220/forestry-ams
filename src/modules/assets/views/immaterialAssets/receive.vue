<template>
	...
	<div class="not_finished">
		<span>修改未结束领用单</span>
		<el-tooltip content="对未“结束领用”的领用单、资产清单进行修改" placement="top">
	...
	<div class="form_box">
		<div class="form_title">新建领用单</div>
		<el-form :model="docInfoForm" :inline="true">
			...
			<!-- 物品清单 -->
			<div class="details">
				<div class="details_title">领用明细</div>
				<div class="btns">
			...
			<el-table :data="goodsList" :stripe="true" style="width: 100%" @selection-change="handleSelectionChange" :border="true" :header-cell-style="{ backgroundColor: '#F5F5F5' }">
				...
				<el-table-column prop="field26" label="领取数量" header-align="center" width="200" align="center">
					<template #default="scope">
						<el-input-number v-model="scope.row.goodsCnt" :min="1" :max="scope.row.surplusCnt ? scope.row.surplusCnt : 0" placeholder="领取数量" />
					</template>
				</el-table-column>
			...
			<el-row class="save_btns">
				...
				<el-tooltip content="选择此项保存数据，后面就不能再对这个领用单进行修改了，请谨慎操作" placement="top">
					<el-button type="danger" @click="save(1)" :icon="Check">保存并结束领用</el-button>
				</el-tooltip>
			</el-row>
		</el-form>
	</div>
	...
	<cl-dialog v-model="addDialog" title="资产选择" center draggable width="950">
		...
		<cl-row>
			...
			>
				...
				<!-- <el-table-column prop="surplusCnt" label="数量" width="150" sortable align="center" /> -->
				<template #empty>
					<el-empty />
				</template>
			</el-table>
...
<script lang="ts" name="immaterialAssetsReceivePage" setup>
...
const save = (status: number) => {
	...
	if (docObj['name'] == null || docObj['name'] == '') {
		ElMessage.info('请填写【领用人】')
		return
	}
	if (docObj['date'] == null || docObj['date'] == '') {
		ElMessage.info('请填写【领用时间】')
		return
	}
	...
	if (routeParamsId.value == undefined || routeParamsId.value == null || routeParamsId.value == '') {
		service.assets.goodsReceive.goodsReceiveDocController.add(docObj).then((res) => {
		...
		service.assets.goodsReceive.goodsReceiveDocController.updateDoc(docObj).then((res) => {
			if (res != null && res.code != null && parseInt(res.code) == 1000) {
				ElMessage.success('保存成功')
				// 重置表单
				resetForm()
