<template>
	<cl-crud ref="Crud">
		<cl-row>
			...
			<cl-refresh-btn />
			<!-- 自定义列 -->
			<cl-column-custom :columns="custom" name="immateralScrapCollect" />
			<!-- 生成报废汇总表 -->
			<el-button type="success" size="default" @click="handleAddDetails" v-permission="service.assets.summary.immaterialAssetsDiscardSummaryController.permission.add">生成报废汇总表</el-button>
...
const Crud = useCrud(
	{
		...
		dict: {
			api: {
				page: 'getImmaterialDiscardGoodsListByPage',
			},
		},
...
const handleSaveAddDetails = async () => {
	...
	service.assets.summary.immaterialAssetsDiscardSummaryController
		.add({
			name: form.name,
