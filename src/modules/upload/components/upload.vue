<template>
	<div class="cl-upload__wrap" :class="[customClass]">
		...
		>
			...
			>
				<template #footer>
					<div class="cl-upload__footer" v-if="(type == 'image' || drag) && isAdd" @click="space.open()">
						<el-upload
							:drag="drag"
							:ref="setRefs('upload')"
							action=""
						...
						>
							<slot>
								<div class="cl-upload__dragger" v-if="drag">
									...
									<div>点击上传或将文件拖动到此处，文件大小限制{{ limitSize }}M</div>
								</div>
					...
					>
						<slot name="item" :item="item" :index="index">
							<div class="cl-upload__item" :class="[`is-${item.type}`]">
								...
								<template v-else>
									<!-- 文件名 -->
									<span class="cl-upload__name">{{ fileName(item.preload) }}.{{ extname(item.preload) }}</span>
								</template>
								...
								<!-- 进度条 -->
								<div class="cl-upload__progress" v-if="item.progress > 0 && item.progress < 100">
...
const isAdd = computed(() => {
	return props.multiple ? !disabled.value && limit - list.value.length > 0 : list.value.length == 0
})
...
async function beforeUpload(file: any, item?: Upload.Item) {
	function next() {
		...
		// 预览
		d.preload = d.url || (d.type == 'image' ? window.webkitURL.createObjectURL(file) : file.name)
