<template>
	...
	<div class="not_finished">
		<span>归还单</span>
		<el-tooltip content="对未“结束归还”的归还单、资产清单进行修改" placement="top">
			<el-select v-model="routeParamsLabel" filterable placeholder="对未结束的归还单进行修改" width="250" @change="selectIncompleteRecord">
	...
	<div class="form_box">
		<div class="form_title">新建归还单</div>
		<el-form :model="docInfoForm" :inline="true">
			...
			<el-row>
				...
				<el-form-item label="资产状态" required>
					<el-select style="width: 220px" v-model="docInfoForm.assetsStatus" placeholder="请选择">
						<el-option label="库存" value="库存"></el-option>
						<el-option label="待处置" value="待处置"></el-option>
			...
			<el-table :data="goodsList" :stripe="true" style="width: 100%" @selection-change="handleSelectionChange" :border="true" :header-cell-style="{ backgroundColor: '#F5F5F5' }">
				...
				<el-table-column prop="goodsCnt" label="归还数量" header-align="center" width="200" align="center">
					<template #default="scope">
						<el-input-number v-model="scope.row.goodsCnt" :min="1" :max="scope.row.surplusCnt ? scope.row.surplusCnt : 0" placeholder="归还数量" />
					</template>
				</el-table-column>
			...
			<el-row class="save_btns">
				...
				<el-tooltip content="选择此项保存数据，后面就不能再对这个归还单进行修改了，请谨慎操作" placement="top">
					<el-button type="danger" @click="save(1)" :icon="Check">保存并结束归还</el-button>
				</el-tooltip>
			</el-row>
		</el-form>
	</div>
	...
	<cl-dialog v-model="addDialog" title="资产选择" center draggable width="950">
		...
		<cl-row>
			...
			>
				...
				<el-table-column prop="cnt" label="待还数量" header-align="center" width="200" align="center">
					<template #default="scope">
						<el-text v-if="curSubject == '固定资产'">{{ scope.row.surplusCnt }}</el-text>
						<el-text v-if="curSubject == '无形资产'">{{ scope.row.surplusCnt }}</el-text>
						<el-text v-if="curSubject == '易耗品'">{{ scope.row.surplusCnt }}</el-text>
...
<script lang="ts" name="goodsTransfer" setup>
...
const docInfoForm: any = ref({
	...
	storagePlace: '', // 存放地点
	assetsStatus: '库存', // 资产状态assetsStatus
	remark: '', // 备注
	...
	if ((startTime != null && startTime != '') || (endTime != null && endTime != '') || (keyWord != null && keyWord != '')) {
		...
		tableDataList = tableDataList.filter((item: any) => {
			...
			if (keyWord != null && keyWord != '') {
				// 关键词 搜索：资产名称、财厅编号
				const name = item['field41'] != null && item['field41'] != '' ? item['field41'] : ''
				const CTNo = item['field3'] != null && item['field3'] != '' ? item['field3'] : ''
			...
			// 入库日期
			const timeText = item['field55'] != null && item['field55'] != '' ? item['field55'] : ''
			let curDate = null
...
// 打开资产选择窗口
const openAddDialog = () => {
	if (docInfoForm.value.userInfo == null || docInfoForm.value.userInfo.id == null || docInfoForm.value.userInfo.id == '') {
		ElMessage.info('请选择【归还人】')
	} else {
...
const getIncompleteRecoeds = () => {
	service.assets.goodsReturn.goodsReturnDocController
		.getIncompleteRecoeds({
		...
		.then((resp) => {
			if (resp != null) {
				if (resp.putStorageDate != null && resp.putStorageDate != '') {
					...
					docInfoForm.value.assetsStatus = resp.assetsStatus ? resp.assetsStatus : ''
	...
	if (docInfoForm.value.userInfo == null || docInfoForm.value.userInfo.id == null || docInfoForm.value.userInfo.id == '') {
		...
		service.assets.goodsReceive.goodsReceiveDocController
			.getUserNoReturnGoodsList({
				dataType: curSubject.value,
				userId: docInfoForm.value.userInfo.id,
			})
			.then((resp: any) => {
				if (resp != null && resp.data != null && resp.data.length > 0) {
					fixedAssetsGoodsList.value = resp.data
					tablePager.value.total = resp.data.length
					// 调用分页
					page()
				} else {
					fixedAssetsGoodsList.value = []
...
const save = (status: number) => {
	let docObj = {
		...
		assetsStatus: docInfoForm.value.assetsStatus,
	}
	if (docInfoForm.value.userInfo.value == docObj.name) docObj['userId'] = docInfoForm.value.userInfo.id

	// 由于需要填写 领取数量，所以不能只取ID，还要把数量带上
	// const goodsIds = goodsList.value != null && goodsList.value.length > 0 ? goodsList.value.map((item: any) => item.id) : []
	const goodsArr =
		goodsList.value != null && goodsList.value.length > 0
			? goodsList.value.map((item: any) => {
					return { id: item.id, goodsCnt: item.goodsCnt, docId: item.receiveDocId, receiveListId: item.receiveListId }
			  })
			: []
	if (docObj['name'] == null || docObj['name'] == '') {
		ElMessage.info('请填写【归还人】')
		return
	}
	if (docObj['date'] == null || docObj['date'] == '') {
		ElMessage.info('请填写【归还日期】')
		return
	}
	...
	if (docObj['assetsStatus'] == null || docObj['assetsStatus'] == '') {
		ElMessage.info('请选择【资产状态】')
		return
	}
	// if (goodsArr == null || goodsArr.length <= 0) {
	// 	ElMessage.info('请添加【归还明细】')
	// 	return
	// }
	docObj['goodsList'] = goodsArr

	if (routeParamsId.value == undefined || routeParamsId.value == null || routeParamsId.value == '') {
		service.assets.goodsReturn.goodsReturnDocController.add(docObj).then((res) => {
		...
		service.assets.goodsReturn.goodsReturnDocController.updateDoc(docObj).then((res) => {
			if (res != null && res.code != null && parseInt(res.code) == 1000) {
				ElMessage.success('保存成功')
				// 重置表单
				resetForm()
