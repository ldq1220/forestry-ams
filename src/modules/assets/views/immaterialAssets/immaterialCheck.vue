<template>
	<cl-crud ref="Crud">
		<cl-row>
			...
			<!-- 跳转到盘点计划新建页面 -->
			<router-link :to="'/assets/addStockCheckPlan?dataType=' + 'immaterial'" style="color: #fff">
				<el-button type="primary">创建盘点计划</el-button>
			</router-link>
			<!-- <cl-add-btn /> -->
			<!-- 导出 -->
			<cl-export-btn :columns="Table?.columns" :data="onExportData" filename="无形资产库存盘点" />
			<!-- 删除按钮 -->
			<!-- <cl-multi-delete-btn /> -->
			<cl-flex1 />
		...
		<cl-dialog :title="'盘点计划:' + planName" v-model="checkDialogVisible" width="950" draggable>
			...
			>
				...
				<el-table-column prop="stockCheckOp" label="盘点操作" width="160" fixed="right">
					<template #default="scope">
			...
			<div class="receive_box">
				...
				<div class="info_dialog_content" v-for="item in assetsInfo" :key="item.id">
					<div class="content_item">
						...
						<div class="item_content" v-if="item.label !== '数量'">{{ item.value }}</div>
						<el-input v-model="goodsCnt" placeholder="请输入盘点数量" v-else></el-input>
					</div>
				</div>
...
<script lang="ts" name="immaterialCheck" setup>
...
const goodsCnt = ref() // 盘点数量
const planName = ref() //盘点计划名称
...
const getAssetsInfo = () => {
	// 在此处编写原始的获取资源信息逻辑
	assetsInfo.value = []
	service.assets.inventory.immaterialAssetsInventoryPlanController
		...
		.then((res) => {
			...
			assetsInfo.value.forEach((item: { label: string; value: any }) => {
				if (item.label === '数量') {
					goodsCnt.value = item.value
				}
			})
		})
}
				...
				return [
					{
						...
						onClick() {
							...
							service.assets.stockCheck.stockCheckController
								...
								.then((res) => {
									let goodsList = res != null && res.goodsList != null ? res.goodsList : null
					...
					},
					{
						...
						onClick() {
							router.push('/assets/addStockCheckPlan?dataType=immaterial&id=' + scope.row.id)
						},
					},
					{
						label: '删除',
...
const onExportData = async () => {
	return service.assets.inventory.immaterialAssetsInventoryPlanController.getPlanByPage().then((res) => res.list)
}
