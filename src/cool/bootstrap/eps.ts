...
function getNames(v: any) {
	return [...Object.getOwnPropertyNames(v.constructor.prototype), ...Object.keys(v)].filter((e) => !['namespace', 'constructor', 'request', 'permission'].includes(e))
}
						...
						if (k) {
							...
							if (arr[i + 1]) {
								...
								if (!d[k].permission) {
									...
									ks.forEach((e) => {
										d[k].permission[e] = `${d[k].namespace.replace('admin/', '')}/${e}`.replace(/\//g, ':')
									})
								}
