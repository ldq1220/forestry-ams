<template>
	...
	<div class="not_finished">
		<span>修改未结束报废申请单报废单</span>
		<el-tooltip content="对未“结束报废”的报废单、资产清单进行修改" placement="top">
	...
	<div class="form_box">
		<div class="form_title">新建报废申请单</div>
		<el-form :model="docInfoForm" :inline="true">
			<el-row>
				<el-form-item label="报废负责人" required>
			...
			<el-row>
				<el-form-item label="处置方式" required>
					<el-select style="width: 192px" v-model="docInfoForm.dealType" placeholder="请选择">
			...
			<!-- 物品清单 -->
			<div class="details">
				<div class="details_title">拟报废明细</div>
				<div class="btns">
			...
			<el-table :data="goodsList" :stripe="true" style="width: 100%" @selection-change="handleSelectionChange" :border="true" :header-cell-style="{ backgroundColor: '#F5F5F5' }">
				...
				<el-table-column prop="field3" label="资产名称" header-align="center" align="center" />
				<el-table-column prop="field4" label="财厅编号" header-align="center" align="center" />
				<el-table-column prop="fieldnull" label="规格型号" header-align="center" align="center" />
				<el-table-column prop="field26" label="报废数量" header-align="center" width="200" align="center">
					<template #default="scope">
						<el-input-number v-model="scope.row.goodsCnt" :min="1" :max="scope.row.surplusCnt ? scope.row.surplusCnt : 0" placeholder="报废数量" />
					</template>
				</el-table-column>
				<el-table-column prop="field35" label="存放地点" header-align="center" align="center" />
			...
			<el-row class="save_btns">
				...
				<el-tooltip content="选择此项保存数据，后面就不能再对这个报废单进行修改了，请谨慎操作" placement="top">
					<el-button type="danger" @click="save(1)" :icon="Check">保存并结束报废申请</el-button>
				</el-tooltip>
			</el-row>
		</el-form>
	</div>
	...
	<cl-dialog v-model="addDialog" title="资产选择" center draggable width="950">
		...
		<cl-row>
			...
			>
				...
				<el-table-column prop="field16" label="使用人" width="120" show-overflow-tooltip sortable align="center" />
				<el-table-column prop="field82" label="使用部门" width="120" show-overflow-tooltip sortable align="left" />
				<el-table-column prop="fieldnull" label="规划型号" width="120" show-overflow-tooltip sortable align="left" />
				<el-table-column prop="field35" label="存放地点" width="120" sortable align="center" />
				<el-table-column prop="field6" label="取得日期" width="120" sortable align="center" />
				<el-table-column prop="field69" label="入库日期" width="120" show-overflow-tooltip sortable align="center" />
				<template #empty>
					<el-empty />
				</template>
			</el-table>
...
const filterData = () => {
	...
	// 关键词
	const keyWord: string = goodsSearParams.value.keyWord
		...
		tableDataList = tableDataList.filter((item: any) => {
			...
			if (keyWord != null && keyWord != '') {
				...
				if (textType != null && textType == '资产编号') {
					...
					checkText = item['field35'] != null && item['field35'] != '' ? item['field35'] : ''
				} else {
			...
			if (dateType != null && dateType == '取得日期') {
				timeText = item['field6'] != null && item['field6'] != '' ? item['field6'] : ''
			} else {
				timeText = item['field69'] != null && item['field69'] != '' ? item['field69'] : ''
			}
			let curDate = null
			...
			if (deptName != null && deptName != '') {
				let checkText = item['field82'] != null && item['field82'] != '' ? item['field82'] : ''
			...
			if (userName != null && userName != '') {
				let checkText = item['field16'] != null && item['field16'] != '' ? item['field16'] : ''
		...
		.then((resp) => {
			if (resp != null) {
				if (resp.putStorageDate != null && resp.putStorageDate != '') {
					...
					docInfoForm.value.dealType = resp.dealType ? resp.dealType : ''
					if (resp.userId) {
						docInfoForm.value.userInfo.id = resp.userId
						docInfoForm.value.userInfo.value = resp.name
					}
					// 采购单
					resp.filePath ? (shoppingListImgUrl.value = resp.filePath) : (shoppingListImgUrl.value = '')
					// 资产清单
					if (resp.goodsList != null && resp.goodsList.length > 0) {
						...
						for (let i = 0; i < resp.goodsList.length; i++) {
							if (resp.goodsList[i]['surplusCnt'] == null || resp.goodsList[i]['surplusCnt'] == '') resp.goodsList[i]['surplusCnt'] = 0
							if (resp.goodsList[i].goodsCnt != null && resp.goodsList[i].goodsCnt != '' && parseInt(resp.goodsList[i].goodsCnt) > 0) {
								resp.goodsList[i]['surplusCnt'] += parseInt(resp.goodsList[i].goodsCnt)
							}
							// 检查一下是否有领用数量，没有要加一个
							if (resp.goodsList[i].goodsCnt == null || resp.goodsList[i].goodsCnt == '' || parseInt(resp.goodsList[i].goodsCnt) <= 0) {
							...
							if (resp.goodsList[i].id != null && !checkIds.includes(resp.goodsList[i].id)) {
								...
								// 找到对应数据， 修改剩余数量
								for (let k = 0; k < fixedAssetsGoodsList.value.length; k++) {
									if (fixedAssetsGoodsList.value[k].id == resp.goodsList[i].id) {
										fixedAssetsGoodsList.value[k].surplusCnt = resp.goodsList[i]['surplusCnt']
										break
									}
								}
							}
...
const save = (status: number) => {
	...
	if (docObj['name'] == null || docObj['name'] == '') {
		ElMessage.info('请填写【报废负责人】')
		return
	}
	if (docObj['date'] == null || docObj['date'] == '') {
		ElMessage.info('请填写【报废时间】')
		return
	}
	if (docObj['dealType'] == null || docObj['dealType'] == '') {
		ElMessage.info('请选择【处置方式】')
		return
	}
	docObj['goodsList'] = goodsArr

	if (routeParamsId.value == undefined || routeParamsId.value == null || routeParamsId.value == '') {
		service.assets.goodsDiscard.goodsDiscardDocController.add(docObj).then((res) => {
		...
		service.assets.goodsDiscard.goodsDiscardDocController.updateDoc(docObj).then((res) => {
			if (res != null && res.code != null && parseInt(res.code) == 1000) {
				ElMessage.success('保存成功')
				// 重置表单
				resetForm()
