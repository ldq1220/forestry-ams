<template>
			...
			<cl-table ref="Table">
				...
				<template #column-goodslist="{ scope }">
					<div>
						...
						<el-table :data="scope.row.goodsList" :stripe="true" style="width: 90%; margin: 0 5% 12px 5%" table-layout="fixed" :border="true">
							...
							<el-table-column prop="field3" label="资产名称" header-align="center" />
							<el-table-column prop="field4" label="财厅编号" header-align="center" />
							<el-table-column prop="field2" label="资产分类" header-align="center" />
							<el-table-column prop="goodsCnt" label="数量" header-align="center" />
							<el-table-column prop="field35" label="存放地点" header-align="center" />
						</el-table>
					</div>
				</template>
				<template #column-op="{ scope }">
					...
					<el-tooltip content="对报废单、资产清单进行修改" placement="top">
						...
						>
							<!-- 修改 -->
							<router-link :to="'/assets/immaterialAssetsDiscard?id=' + scope.row.id">修改</router-link>
						</el-button>
					</el-tooltip>
	...
	<!-- 明细 -->
	<immateralScrapCollect v-else-if="flag2" />
	<!-- 汇总 -->
	<immaterialScrapSummary v-else />
	<!-- 打印 -->
...
<script lang="ts" name="immaterialScrapDetails" setup>
import { useCrud, useTable } from '@cool-vue/crud'
import { useCool } from '/@/cool'
import { ref, reactive } from 'vue'
import dayjs from 'dayjs'
import { type TabsPaneContext } from 'element-plus'
import immateralScrapCollect from './components/immateralScrapCollect.vue'
import immaterialScrapSummary from './components/immaterialScrapSummary.vue'
...
const Crud = useCrud(
	{
		...
		dict: {
			api: {
				page: 'getImmaterialAssetsDiscardDocByPage',
			},
		},
