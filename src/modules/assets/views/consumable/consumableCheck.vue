<template>
	<cl-crud ref="Crud">
		<cl-row>
			...
			<!-- 跳转到盘点计划新建页面 -->
			<router-link :to="'/assets/addStockCheckPlan?dataType=' + 'consumable'" style="color: #fff">
...
const getAssetsInfo = () => {
	// 在此处编写原始的获取资源信息逻辑
	assetsInfo.value = []
	service.assets.inventory.consumableAssetsInventoryPlanController
			...
			buttons({ scope }) {
				return [
					...
					},
					{
						label: '结束盘点',
						type: 'warning',
						hidden: scope.row.status !== 0 || !checkPerm(service.assets.stockCheck.stockCheckController.permission.updateStatus as any),
						onClick() {
					...
					},
					{
						...
						onClick() {
							router.push('/assets/addStockCheckPlan?dataType=consumable&id=' + scope.row.id)
						},
					},
					{
						label: '删除',
