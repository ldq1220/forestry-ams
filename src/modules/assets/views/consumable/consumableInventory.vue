	...
	<cl-crud ref="Crud">
		<cl-row>
			...
			<cl-filter label="资产状态">
				...
				<cl-select style="width: 120px" :options="options.status" prop="field6" />
			</cl-filter>
		...
		<!-- 批量修改对话框 -->
		<batchUpdateDialog :visiable="visiable" dataType="易耗品" :selectionChangeData="selectionChangeData" @closeDialog="closeDialog" />
					...
					<el-tab-pane label="基础信息" name="basics">
						<div class="info_dialog_content">
							<div class="content_item" v-for="(item, index) in fixedAssetsInfo.baseData" :key="index">
								...
								<div class="item_content" v-if="item.label !== '采购单'">{{ item.value }}</div>
								<div class="item_content" v-else>
									<a :href="item.value" target="_blank" v-if="item.value" class="look_text">查看</a>
								</div>
							</div>
						</div>
					</el-tab-pane>
...
<script lang="ts" name="consumableInventory" setup>
				...
				buttons({ scope }) {
					return [
						'edit',
						{
							...
							type: 'primary',
							onClick() {
								service.assets.consumable.consumableAssetsGoodsController.getConsumableGoodsInfoById({ id: scope.row.id }).then((res) => {
...
const Upsert = useUpsert({
	...
	onInfo(data, { done, close }) {
		...
		// 使用其他接口
		service.assets.consumable.consumableAssetsGoodsController.getConsumableGoodsInfo({ id: data.id }).then((res) => {
			done(res)
		})
...
const Crud = useCrud(
	{
		...
		dict: {
			api: {
				...
				list: 'queryAll',
				add: 'addConsumableGoodsInfo',
				update: 'updateConsumableGoodsInfo',
				delete: 'deleteConsumableGoodsInfo',
				info: 'getConsumableGoodsInfo',
			},
			label: {
				add: '入库',
			},
