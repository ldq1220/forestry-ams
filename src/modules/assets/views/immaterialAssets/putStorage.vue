	...
	<div class="form_box">
		...
		<el-form :model="docInfoForm" :inline="true">
			...
			>
				...
				<el-table-column prop="field2" label="资产分类" header-align="center" align="center" />
				...
				<el-table-column prop="field11" label="资产原值(元)" header-align="center" align="center" width="120" />
...
<script lang="ts" name="immaterialAssetsPutStorage" setup>
...
const setDefaultValue4Field = (item: any) => {
	...
	if (item.fieldName != null) {
		...
		if (item.fieldName == '单位会计科目' && (item.value == null || item.value == '')) item.value = '无形资产'
...
const addToList = (goods: any) => {
	...
	// 数量
	const field26Value: number = goods['field26'] == null || goods['field26'] == '' || parseInt(goods['field26']) <= 0 ? 1 : parseInt(goods['field26'])
	if (goods['cnt'] == null || goods['cnt'] == '') {
		goods['cnt'] = field26Value
	}
	if (goods['amount'] == null || goods['amount'] == '') {
		goods['amount'] = field26Value
	}
	if (goods['field4'] != null && goods['field4'] != '') {
		let assetsNoArr = goodsList.value.map((obj: any) => obj['field4'])
		// 检查一下资产编号，财厅编号如果已经存在于列表之中，那就不要再放进去了
		if (!assetsNoArr.includes(goods['field4'])) {
			assetsNoArr.push(goods['field4'])
