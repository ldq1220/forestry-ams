<template>
	<el-tabs v-model="activeName" class="demo-tabs" @tab-click="handleClick">
		<el-tab-pane label="&nbsp;&nbsp;&nbsp;&nbsp;报废单&nbsp;&nbsp;&nbsp;&nbsp;" name="first"></el-tab-pane>
	...
	<cl-crud ref="Crud" v-if="flag">
		<cl-row>
			...
			<div class="date_picker">
				<span style="font-size: 12px; margin-right: 10px">报废日期</span>
				<el-date-picker
			...
			<!-- 关键字搜索 -->
			<cl-search-key placeholder="搜索 报废负责人/备注" />
		</cl-row>
			...
			<cl-table ref="Table">
				...
				<template #column-goodslist="{ scope }">
					<div>
						...
						<el-table :data="scope.row.goodsList" :stripe="true" style="width: 90%; margin: 0 5% 12px 5%" table-layout="fixed" :border="true">
							...
							<el-table-column prop="field41" label="资产名称" header-align="center" align="left" width="300" fixed />
							<el-table-column prop="field23" label="数量" header-align="center" align="center" width="150" />
							<el-table-column prop="field78" label="规划型号" header-align="center" align="left" width="200" />
							<el-table-column prop="field12" label="资产状态" header-align="center" align="center" width="150" />
							<el-table-column prop="field2" label="资产分类" header-align="center" align="left" width="300" />
							<el-table-column prop="field32" label="存放地点" header-align="center" align="center" width="200" />
							<el-table-column prop="field49" label="管理部门" header-align="center" align="center" width="300" />
							<el-table-column prop="field15" label="管理人" header-align="center" align="center" width="150" />
							<el-table-column prop="field55" label="入库日期" header-align="center" align="center" width="150">
				...
				<template #column-op="{ scope }">
					...
					<el-tooltip content="导出" placement="top">
						<cl-export-btn link type="primary" size="small" :columns="exportColsArr" :data="getExportExcelData(scope.row)" :filename="'报废单导出-' + new Date().getTime()" />
					</el-tooltip>
					<!-- 结束报废 -->
					<el-tooltip content="结束报废，确定结束之后，将不能再对这个报废单进行修改" placement="top">
						<el-button
							link
							type="danger"
							size="small"
							...
							v-permission="service.assets.goodsDiscard.goodsDiscardDocController.permission.updateStatus"
						>
							结束报废
						</el-button>
					</el-tooltip>
					<el-tooltip content="对报废单、资产清单进行修改" placement="top">
						<el-button
							link
							size="small"
							v-if="scope.row.status == null || parseInt(scope.row.status) != 1"
							v-permission="service.assets.goodsDiscard.goodsDiscardDocController.permission.updateDoc"
						>
							<!-- 修改 -->
							<router-link :to="'/assets/fixedAssetsDiscard?id=' + scope.row.id">修改</router-link>
						</el-button>
					</el-tooltip>
					<el-tooltip content="删除报废单" placement="top">
						<el-button link type="danger" size="small" @click="deleteDoc(scope.row)" v-permission="service.assets.goodsDiscard.goodsDiscardDocController.permission.deleteDoc">
							删除
						</el-button>
					</el-tooltip>
				</template>
	...
	<!-- 明细 -->
	<fixedScrapCollect v-else-if="flag2" />
	<!-- 汇总 -->
	<fixedScrapSummary v-else />
	<!-- 打印 -->
	<el-form :model="printItem.item" style="position: absolute; top: -99999px; left: -99999px">
		<div ref="print_dom" id="print_dom">
			<h2 style="width: 100%; text-align: center">报废单</h2>
			<div>
				<table style="width: 100%; border: 1px solid #ddd; border-collapse: collapse">
					<tr>
						<th style="width: 150px; border: 1px solid #ddd; padding: 12px 0">报废负责人　</th>
						<td style="border: 1px solid #ddd; padding: 12px">{{ printItem.item.name }}</td>
					</tr>
					<tr>
						<th style="width: 150px; border: 1px solid #ddd; padding: 12px 0">报废时间</th>
					...
					<el-row v-for="(item, index) in printItem.item.goodsList" :key="index">
						<el-row v-if="item.accessoryList != null && item.accessoryList.length > 0">
							<tr style="width: 100%; border: 1px solid #ddd">
								...
								<td style="border: 1px solid #ddd; padding: 12px">{{ item.field41 }}</td>
								<td style="border: 1px solid #ddd; padding: 12px 0; text-align: center">{{ item.field3 }}</td>
								<td style="border: 1px solid #ddd; padding: 12px 0; text-align: center">{{ item.field78 }}</td>
								<td style="width: 100px; border: 1px solid #ddd; padding: 12px 0; text-align: center">{{ item.field23 }}</td>
								<td style="border: 1px solid #ddd; padding: 12px 0; text-align: center">{{ item.field32 }}</td>
							</tr>
							<tr style="width: 100%; border: 1px solid #ddd">
								<td colspan="6">
									<table
...
<script lang="ts" name="fixedAssetsScrapDetails" setup>
import { useCrud, useTable } from '@cool-vue/crud'
import { useCool } from '/@/cool'
import { ref, reactive } from 'vue'
import dayjs from 'dayjs'
import { type TabsPaneContext } from 'element-plus'
import fixedScrapCollect from './commponents/fixedScrapCollect.vue'
import fixedScrapSummary from './commponents/fixedScrapSummary.vue'
...
const putStorageTime: any = ref([]) // 报废日期
// 报废日期搜素
...
const Crud = useCrud(
	{
		service: service.assets.goodsDiscard.goodsDiscardDocController,
		dict: {
			api: {
				page: 'getFixedAssetsDiscardDocByPage',
			},
		},
		async onRefresh(params, { next, render }) {
			if (putStorageTime.value != null && putStorageTime.value.length > 0) {
				params['putStorageStartDate'] = dayjs(putStorageTime.value[0]).format('YYYY-MM-DD') // 报废日期（开始）
				params['putStorageEndDate'] = dayjs(putStorageTime.value[1]).format('YYYY-MM-DD') // 报废日期（结束）
			} else {
				params['putStorageStartDate'] = undefined // 报废日期（开始）
				params['putStorageEndDate'] = undefined // 报废日期（结束）
			}
	...
	if (item != null && item.goodsList != null && item.goodsList.length > 0) {
		for (let i = 0; i < item.goodsList.length; i++) {
			...
			data.push({
				...
				col4: goods.field41 ? goods.field41 : '',
				col5: goods.field23 ? goods.field23 : '',
				col6: goods.field78 ? goods.field78 : '',
				col7: goods.field12 ? goods.field12 : '',
				col8: goods.field2 ? goods.field2 : '',
				col9: goods.field32 ? goods.field32 : '',
				col10: goods.field49 ? goods.field49 : '',
				col11: goods.field15 ? goods.field15 : '',
				col12: goods.field55 ? goods.field55 : '',
			})
...
const updateStatus = (item: any) => {
	ElMessageBox.confirm('确定要结束报废吗?', '确认操作', {
		confirmButtonText: '确定',
		cancelButtonText: '取消',
		type: 'warning',
	})
		.then(() => {
			service.assets.goodsDiscard.goodsDiscardDocController
				.updateStatus({
					id: item.id.toString(),
					status: 1,
				})
...
const deleteDoc = (item: any) => {
	ElMessageBox.confirm('确定要删除报废单吗?', '确认操作', {
		confirmButtonText: '确定',
		cancelButtonText: '取消',
		type: 'warning',
	})
