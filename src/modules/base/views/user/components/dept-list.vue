				...
				>
					<template #default="{ node, data }">
						<div class="dept-tree__node">
							...
							<span v-if="browser.isMini" class="dept-tree__node-icon" @click="onContextMenu($event, data, node)">
								<el-icon>
									<more-filled />
								</el-icon>
							</span>
		...
		await service.base.sys.department
			...
			.then(() => {
				...
				if (f) {
					...
					ElMessageBox.confirm(`“${item.name}” 部门的用户已成功转移到 “${item.parentName}” 部门。`, '删除成功')
				}
			})
