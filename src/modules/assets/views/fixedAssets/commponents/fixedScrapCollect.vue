<template>
	<cl-crud ref="Crud">
		<cl-row>
			...
			<cl-refresh-btn />
			<!-- 自定义列 -->
			<cl-column-custom :columns="custom" name="fixedScrapCollect" />
			<!-- 生成报废汇总表 -->
			<el-button type="success" size="default" @click="handleAddDetails" v-permission="service.assets.summary.fixedAssetsDiscardSummaryController.permission.add">生成报废汇总表</el-button>
	...
	<!-- 生成报废汇总表 -->
	<cl-dialog title="生成报废汇总表" v-model="addDetailsDialog" width="30%" center draggable>
...
<script lang="ts" name="fixedScrapCollect" setup>
...
const Crud = useCrud(
	{
		...
		dict: {
			api: {
				page: 'getFixedDiscardGoodsListByPage',
			},
		},
		async onRefresh(params, { next, render }) {
			if (putStorageTime.value != null && putStorageTime.value.length > 0) {
				params['startTime'] = dayjs(putStorageTime.value[0]).format('YYYY-MM-DD') // 报废日期（开始）
				params['endTime'] = dayjs(putStorageTime.value[1]).format('YYYY-MM-DD') // 报废日期（结束）
			} else {
				params['startTime'] = undefined // 报废日期（开始）
				params['endTime'] = undefined // 报废日期（结束）
			}
...
const form: any = reactive({
	...
	rules: {
		...
		date: [{ required: true, message: '请选择报废日期', trigger: 'blur' }],
	},
	userInfo: {},
})
...
const handleAddDetails = () => {
	if (checkTableData.value.length === 0) return ElMessage.warning('请选择报废汇总项')
...
const handleSaveAddDetails = async () => {
	...
	service.assets.summary.fixedAssetsDiscardSummaryController
		.add({
			name: form.name,
