<template>
	<el-tabs v-model="activeName" class="demo-tabs" @tab-click="handleClick">
		<el-tab-pane label="&nbsp;&nbsp;&nbsp;&nbsp;领用单&nbsp;&nbsp;&nbsp;&nbsp;" name="first"></el-tab-pane>
	...
	<cl-crud ref="Crud" v-if="flag">
		<cl-row>
			...
			<!-- 关键字搜索 -->
			<cl-search-key placeholder="搜索 领用人/备注" />
		</cl-row>
			...
			<cl-table ref="Table">
				...
				<template #column-goodslist="{ scope }">
					<div>
						...
						<el-table :data="scope.row.goodsList" :stripe="true" style="width: 90%; margin: 0 5% 12px 5%" table-layout="fixed" :border="true">
							...
							<el-table-column prop="field55" label="领用日期" header-align="center" align="center" width="150">
				...
				<template #column-op="{ scope }">
					...
					<el-tooltip content="导出" placement="top">
						<cl-export-btn link type="primary" size="small" :columns="exportColsArr" :data="getExportExcelData(scope.row)" :filename="'领用单导出-' + new Date().getTime()" />
					</el-tooltip>
					<!-- 结束领用 -->
					<el-tooltip content="结束领用，确定结束之后，将不能再对这个领用单进行修改" placement="top">
						<el-button
							link
							type="danger"
							size="small"
							...
							v-permission="service.assets.goodsReceive.goodsReceiveDocController.permission.updateStatus"
						>
							结束领用
						</el-button>
					</el-tooltip>
					<el-tooltip content="对领用单、资产清单进行修改" placement="top">
						<el-button
							link
							size="small"
							v-if="scope.row.status == null || parseInt(scope.row.status) != 1"
							v-permission="service.assets.goodsReceive.goodsReceiveDocController.permission.updateDoc"
						>
							<!-- 修改 -->
							<router-link :to="'/assets/fixedAssetsReceive?id=' + scope.row.id">修改</router-link>
						</el-button>
					</el-tooltip>
					<el-tooltip content="删除领用单" placement="top">
						<el-button link type="danger" size="small" @click="deleteDoc(scope.row)" v-permission="service.assets.goodsReceive.goodsReceiveDocController.permission.deleteDoc">
							删除
						</el-button>
					</el-tooltip>
				</template>
	...
	<!-- 明细 -->
	<fixedAssetsReceiveCollect v-else-if="flag2" />
	<!-- 汇总 -->
	<fixedAssetsReceiveSummary v-else />
	<!-- 打印 -->
	<el-form :model="printItem.item" style="position: absolute; top: -99999px; left: -99999px">
		<div ref="print_dom" id="print_dom">
			<h2 style="width: 100%; text-align: center">领用单</h2>
			<div>
				<table style="width: 100%; border: 1px solid #ddd; border-collapse: collapse">
					<tr>
						<th style="width: 150px; border: 1px solid #ddd; padding: 12px 0">领用人　</th>
						<td style="border: 1px solid #ddd; padding: 12px">{{ printItem.item.name }}</td>
					</tr>
					<tr>
						<th style="width: 150px; border: 1px solid #ddd; padding: 12px 0">领用时间</th>
...
<script lang="ts" name="fixedAssetsReceiveDetails" setup>
import { useCrud, useTable } from '@cool-vue/crud'
import { useCool } from '/@/cool'
import { ref, reactive } from 'vue'
import dayjs from 'dayjs'
import { type TabsPaneContext } from 'element-plus'
import fixedAssetsReceiveCollect from './commponents/fixedAssetsReceiveCollect.vue'
import fixedAssetsReceiveSummary from './commponents/fixedAssetsReceiveSummary.vue'
...
const Crud = useCrud(
	{
		...
		dict: {
			api: {
				page: 'getFixedAssetsReceiveDocByPage',
			},
		},
		async onRefresh(params, { next, render }) {
			if (putStorageTime.value != null && putStorageTime.value.length > 0) {
				params['putStorageStartDate'] = dayjs(putStorageTime.value[0]).format('YYYY-MM-DD') // 领用日期（开始）
				params['putStorageEndDate'] = dayjs(putStorageTime.value[1]).format('YYYY-MM-DD') // 领用日期（结束）
			} else {
				params['putStorageStartDate'] = undefined // 领用日期（开始）
				params['putStorageEndDate'] = undefined // 领用日期（结束）
			}
...
const updateStatus = (item: any) => {
	ElMessageBox.confirm('确定要结束领用吗?', '确认操作', {
		confirmButtonText: '确定',
		cancelButtonText: '取消',
		type: 'warning',
	})
		.then(() => {
			service.assets.goodsReceive.goodsReceiveDocController
				.updateStatus({
					id: item.id.toString(),
					status: 1,
				})
...
const deleteDoc = (item: any) => {
	ElMessageBox.confirm('确定要删除领用单吗?', '确认操作', {
		confirmButtonText: '确定',
		cancelButtonText: '取消',
		type: 'warning',
	})
