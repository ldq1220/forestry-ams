<template>
	<cl-crud ref="Crud">
		<cl-row>
			...
			<cl-refresh-btn />
			<!-- 自定义列 -->
			<cl-column-custom :columns="custom" name="consumableWarehousingCollect" />
			<!-- 生成入库汇总表 -->
			<el-button type="success" size="default" @click="handleAddDetails" v-permission="service.assets.summary.consumableAssetsInventorySummaryController.permission.add">
				生成入库汇总表
			</el-button>
...
<script lang="ts" name="consumableWarehousingCollect" setup>
...
const Crud = useCrud(
	{
		...
		dict: {
			api: {
				page: 'getConsumablePutStorageGoodsListByPage',
			},
		},
...
const handleSaveAddDetails = async () => {
	...
	service.assets.summary.consumableAssetsInventorySummaryController
		.add({
			name: form.name,
