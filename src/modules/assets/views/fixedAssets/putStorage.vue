<template>
	...
	<div class="form_box">
		...
		<el-form :model="docInfoForm" :inline="true">
			...
			<!-- 入库明细 -->
			<div class="details">
				<div class="details_title">入库明细</div>
				<div class="btns">
			...
			>
				...
				<el-table-column prop="field41" label="资产名称" header-align="center" align="left" />
				<el-table-column prop="field3" label="财厅编号" header-align="center" align="left" width="200" />
				<el-table-column prop="field78" label="规格型号" header-align="center" align="center" />
				<el-table-column prop="field23" label="数量" header-align="center" align="center" width="100" />
				<el-table-column prop="field32" label="存放地点" header-align="center" align="center" />
				<el-table-column prop="field9" label="资产原值(元)" header-align="center" align="center" width="120" />
...
<script lang="ts" name="fixedAssetsPushStorage" setup>
...
const setDefaultValue4Field = (item: any) => {
	...
	if (item.fieldName != null) {
		...
		if (item.fieldName == '单位会计科目' && (item.value == null || item.value == '')) item.value = '固定资产'
...
const addToList = (goods: any) => {
	...
	if (goods['field3'] != null && goods['field3'] != '') {
		let assetsNoArr = goodsList.value.map((obj: any) => obj['field3'])
		// 检查一下资产编号，财厅编号如果已经存在于列表之中，那就不要再放进去了
		if (!assetsNoArr.includes(goods['field3'])) {
			assetsNoArr.push(goods['field3'])
...
// let clientWidth: any = getClientWidth()
let clientHeight: any = getClientHeight()
const elTableHeight = ref(clientHeight - 480)
window.addEventListener('resize', () => {
	clientHeight = getClientHeight()
	elTableHeight.value = clientHeight - 480
})
