<template>
				...
				<template #column-goodslist="{ scope }">
					<div>
						...
						<el-table :data="scope.row.goodsList" :stripe="true" style="width: 90%; margin: 0 5% 12px 5%" table-layout="fixed" :border="true">
							...
							<el-table-column prop="goodsCnt" label="领取数量" header-align="center" align="center" width="150" />
							<el-table-column prop="field20" label="规划型号" header-align="center" align="left" width="200" />
							<el-table-column prop="field6" label="资产状态" header-align="center" align="center" width="150" />
							<el-table-column prop="field2" label="资产分类" header-align="center" align="left" width="300" />
							<el-table-column prop="field15" label="存放地点" header-align="center" align="center" width="200" />
							<el-table-column prop="field34" label="管理部门" header-align="center" align="center" width="300" />
							<el-table-column prop="field9" label="管理人" header-align="center" align="center" width="150" />
							<el-table-column prop="field19" label="入库日期" header-align="center" align="center" width="150">
				...
				<template #column-op="{ scope }">
					...
					<el-tooltip content="对领用单、资产清单进行修改" placement="top">
						<el-button
							...
							size="small"
							v-if="scope.row.status == null || parseInt(scope.row.status) != 1"
							v-permissio="service.assets.goodsReceive.goodsReceiveDocController.permission.updateDoc"
						>
							<!-- 修改 -->
							<router-link :to="'/assets/consumableAssetsReceive?id=' + scope.row.id">修改</router-link>
						</el-button>
					</el-tooltip>
	...
	<!-- 明细 -->
	<consumableReceiveCollect v-else-if="flag2" />
	<!-- 汇总 -->
	<consumableReceiveSummary v-else />
	<!-- 打印 -->
			...
			<div>
				<table :model="printItem.item.goodsList" style="width: 100%; border: 1px solid #ddd; border-collapse: collapse">
					<tr style="width: 100%; border: 1px solid #ddd">
						...
						<th style="width: 100px; border: 1px solid #ddd; padding: 12px 0">领取数量</th>
					...
					<el-row v-for="(item, index) in printItem.item.goodsList" :key="index">
						<el-row v-if="item.accessoryList != null && item.accessoryList.length > 0">
							<tr style="width: 100%; border: 1px solid #ddd">
								...
								<td style="border: 1px solid #ddd; padding: 12px 0; text-align: center">{{ item.field20 }}</td>
								<td style="width: 100px; border: 1px solid #ddd; padding: 12px 0; text-align: center">{{ item.goodsCnt }}</td>
								<td style="border: 1px solid #ddd; padding: 12px 0; text-align: center">{{ item.field15 }}</td>
							</tr>
							<tr style="width: 100%; border: 1px solid #ddd">
								<td colspan="6">
									<table
...
<script lang="ts" name="consumableReceiveDetails" setup>
import { useCrud, useTable } from '@cool-vue/crud'
import { useCool } from '/@/cool'
import { ref, reactive } from 'vue'
import dayjs from 'dayjs'
import { type TabsPaneContext } from 'element-plus'
import consumableReceiveCollect from './components/consumableReceiveCollect.vue'
import consumableReceiveSummary from './components/consumableReceiveSummary.vue'
...
const Crud = useCrud(
	{
		...
		dict: {
			api: {
				page: 'getConsumableAssetsReceiveDocByPage',
			},
		},
	...
	if (item != null && item.goodsList != null && item.goodsList.length > 0) {
		for (let i = 0; i < item.goodsList.length; i++) {
			...
			data.push({
				...
				col5: goods.goodsCnt ? goods.goodsCnt : '',
				col6: goods.field20 ? goods.field20 : '',
				col7: goods.field6 ? goods.field6 : '',
				col8: goods.field2 ? goods.field2 : '',
				col9: goods.field15 ? goods.field15 : '',
				col10: goods.field34 ? goods.field34 : '',
				col11: goods.field9 ? goods.field9 : '',
				col12: goods.field19 ? (goods.field19.indexOf(' ') != -1 ? goods.field19.split(' ')[0] : goods.field19) : '',
			})
