<template>
	<cl-crud ref="Crud">
		<cl-row>
			...
			<cl-refresh-btn />
			<!-- 自定义列 -->
			<cl-column-custom :columns="custom" name="immaterialWarehousingCollect" />
			<!-- 生成入库汇总表 -->
			<el-button type="success" size="default" @click="handleAddDetails" v-permission="service.assets.summary.immaterialAssetsInventorySummaryController.permission.add">
				生成入库汇总表
			</el-button>
...
<script lang="ts" name="immaterialWarehousingCollect" setup>
...
const Crud = useCrud(
	{
		...
		dict: {
			api: {
				page: 'getImmaterialPutStorageGoodsListByPage',
			},
		},
...
const handleSaveAddDetails = async () => {
	...
	service.assets.summary.immaterialAssetsInventorySummaryController
		.add({
			name: form.name,
