<template>
	<cl-crud ref="Crud">
		<cl-row>
			...
			<cl-refresh-btn />
			<!-- 自定义列 -->
			<cl-column-custom :columns="custom" name="consumableReceiveCollect" />
			<!-- 生成领用汇总表 -->
			<el-button type="success" size="default" @click="handleAddDetails" v-permission="service.assets.summary.consumableAssetsGetSummaryController.permission.add">生成领用汇总表</el-button>
...
<script lang="ts" name="consumableReceiveCollect" setup>
...
const Crud = useCrud(
	{
		...
		dict: {
			api: {
				page: 'getConsumableReceiveGoodsListByPage',
			},
		},
...
const handleSaveAddDetails = async () => {
	...
	// return
	service.assets.summary.consumableAssetsGetSummaryController
		.add({
			name: form.name,
