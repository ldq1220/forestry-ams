<template>
	<cl-view-group ref="ViewGroup">
		<template #item-name="{ item }">{{ item.name }} - {{ item.tableName }} - {{ item.className }}</template>
		...
		<template #right>
			<div class="source" v-loading="loading">
				...
				<div class="footer">
					<el-pagination background layout="prev, pager, next" :current-page="pagination.page" :total="pagination.total" :page-size="pagination.size" @current-change="onCurrentChange" />
				</div>
...
// 清空数据
function clear() {
	ElMessageBox.confirm(`此操作将会清空表（${ViewGroup.value?.selected?.name}）的所有数据，是否继续？`, '提示', {
		type: 'warning',
	})
		.then(() => {
			request('clear')
