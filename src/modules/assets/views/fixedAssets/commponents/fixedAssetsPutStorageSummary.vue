				...
				<template #column-detail="{ scope }">
					<div style="padding: 0 10px">
						<el-table :data="scope.row.goodsList" :stripe="true" :border="true" style="width: 90%; margin: 0 5% 12px 5%">
							<el-table-column label="资产名称" prop="field41" header-align="center" />
							<el-table-column label="资产分类" prop="field2" header-align="center" />
							<el-table-column label="财厅编号" prop="field3" header-align="center" />
							<el-table-column label="规格型号" prop="field78" header-align="center" />
							<el-table-column label="存放地点" prop="field32" header-align="center" />
						</el-table>
					</div>
				</template>
				<template #column-op="{ scope }">
					<!-- 导出 -->
					<cl-export-btn link type="primary" size="small" :columns="exportColumns" :data="getExportExcelData(scope.row)" :filename="'入库汇总导出-' + new Date().getTime()" />
				</template>
			</cl-table>
		</cl-row>
...
<script lang="ts" name="fixedAssetsPutStorageSummary" setup>
...
const Crud = useCrud(
	{
		service: service.assets.summary.fixedAssetsInventorySummaryController,
		dict: {
			api: {
				page: 'page',
			},
...
const getExportExcelData = (item: any) => {
	...
	if (item != null && item.goodsList != null && item.goodsList.length > 0) {
		for (let i = 0; i < item.goodsList.length; i++) {
			...
			data.push({
				col1: i + 1,
				col2: goods.field41 ? goods.field41 : '',
				col3: goods.field2 ? goods.field2 : '',
				col4: goods.field3 ? goods.field3 : '',
				col5: goods.field78 ? goods.field78 : '',
				col6: goods.field32 ? goods.field32 : '',
			})
		}
	}
	return
