<template>
	<cl-crud ref="Crud">
		<cl-row>
			...
			<cl-refresh-btn />
			<!-- 自定义列 -->
			<cl-column-custom :columns="custom" name="fixedAssetsReceiveCollect" />
			<!-- 生成领用汇总表 -->
			<el-button type="success" size="default" @click="handleAddDetails" v-permission="service.assets.summary.fixedAssetsGetSummaryController.permission.add">生成领用汇总表</el-button>
...
<script lang="ts" name="fixedAssetsReceiveCollect" setup>
...
const Crud = useCrud(
	{
		...
		dict: {
			api: {
				page: 'getFixedReceiveGoodsListByPage',
			},
		},
...
const handleSaveAddDetails = async () => {
	...
	service.assets.summary.fixedAssetsGetSummaryController
		.add({
			name: form.name,
