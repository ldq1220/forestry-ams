				...
				<template #column-detail="{ scope }">
					<div style="padding: 0 10px">
						<el-table :data="scope.row.goodsList" :stripe="true" :border="true" style="width: 90%; margin: 0 5% 12px 5%">
							...
							<el-table-column label="管理人" prop="field9" header-align="center" />
							<el-table-column label="规格型号" prop="field20" header-align="center" />
							<el-table-column label="存放地点" prop="field15" header-align="center" />
						</el-table>
					</div>
				</template>
...
<script lang="ts" name="consumableWarehousingCollectSummary" setup>
...
const Crud = useCrud(
	{
		service: service.assets.summary.consumableAssetsInventorySummaryController,
		dict: {
			api: {
				page: 'page',
			},
	...
	if (item != null && item.goodsList != null && item.goodsList.length > 0) {
		for (let i = 0; i < item.goodsList.length; i++) {
			...
			data.push({
				...
				col4: goods.field9 ? goods.field9 : '',
				col5: goods.field20 ? goods.field20 : '',
				col6: goods.field15 ? goods.field15 : '',
			})
		}
	}
	return data
